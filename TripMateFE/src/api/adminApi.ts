...
export const adminApi = {
  ...
  getPendingVerifications: async (): Promise<ApiResponse<PendingHostVerification[]>> => {
    const res = await axiosClient.get<ApiResponse<PendingHostVerification[]>>('/api/users/host-verifications/pending');
    return res.data;
  },
  ...
  approveHostVerification: async (userId: string): Promise<ApiResponse<{ isSuccess: boolean }>> => {
    const res = await axiosClient.post<ApiResponse<{ isSuccess: boolean }>>(`/api/users/host-verifications/${userId}/approve`);
    return res.data;
  },
  ...
  rejectHostVerification: async (userId: string, reason?: string): Promise<ApiResponse<{ isSuccess: boolean }>> => {
    const res = await axiosClient.post<ApiResponse<{ isSuccess: boolean }>>(`/api/users/host-verifications/${userId}/reject`, { reason });
    return res.data;
  },

  // 5. Lấy danh sách tất cả người dùng trong hệ thống
  getUsers: async (): Promise<ApiResponse<AdminUserListItem[]>> => {
    const res = await axiosClient.get<ApiResponse<AdminUserListItem[]>>('/api/users');
    return res.data;
  },

  // 6. Khóa hoặc Mở khóa tài khoản người dùng
  toggleUserStatus: async (userId: string): Promise<ApiResponse<{ isSuccess: boolean }>> => {
    const res = await axiosClient.post<ApiResponse<{ isSuccess: boolean }>>(`/api/users/${userId}/toggle-status`);
    return res.data;
  },
