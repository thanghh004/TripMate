    ...
    return (
      <div className={`flex flex-col gap-1.5 w-full text-left ${containerClassName}`}>
        ...
        <div className="relative flex items-center w-full">
          {effectiveLeftIcon && (
            <span className="absolute left-3.5 text-slate-400 pointer-events-none flex items-center shrink-0">
              {effectiveLeftIcon}
            </span>
          )}
          <input
          ...
          {!isPassword && rightIcon && (
            <span className="absolute right-3.5 text-slate-400 flex items-center shrink-0">{rightIcon}</span>
          )}
        </div>
